...
def draw_progress_bar(surface, trial_idx, total_trials, block_idx, total_blocks):
    ...
    label = small_font.render(f"B{block_idx+1}  {trial_idx+1}/{total_trials}", True, COLOR_WHITE)
...
def run_block_complete_screen(surface, virtual_screen, block_idx, total_blocks):
    ...
    while pygame.time.get_ticks() - start < duration_ms:
        ...
        congrats2 = font.render(f"还剩 {total_blocks - block_idx - 1} 个Block", True, COLOR_WHITE)
        surface.blit(congrats1, (GAME_WIDTH * SCALE // 2 - congrats1.get_width() // 2, 180))
        surface.blit(congrats2, (GAME_WIDTH * SCALE // 2 - congrats2.get_width() // 2, 230))
