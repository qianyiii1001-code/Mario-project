...
if FONT_PATH:
    ...
    font = safe_sysfont("SimHei", 24)
    small_font = safe_sysfont("SimHei", 18)
...
def draw_retro_scenery_v2(surface):
    """Block 2 专用背景：山丘 + 草地 + 食人花"""
    COLOR_SKY_V2 = (107, 168, 252)
    COLOR_MOUNTAIN_DARK = (34, 100, 34)
    COLOR_MOUNTAIN_MID = (45, 130, 45)
    COLOR_GRASS_TOP = (60, 180, 60)
    COLOR_GRASS_DARK = (40, 140, 40)
    COLOR_DIRT = (139, 90, 43)
    COLOR_DIRT_DARK = (100, 60, 20)
    COLOR_PIPE_GREEN = (0, 168, 0)
    COLOR_PIPE_DARK = (0, 110, 0)
    COLOR_PIPE_LIGHT = (80, 220, 80)
    COLOR_FLOWER_RED = (220, 40, 40)
    COLOR_FLOWER_WHITE = (255, 255, 255)
    COLOR_FLOWER_STEM = (0, 168, 0)
    COLOR_FLOWER_MOUTH = (255, 180, 180)

    surface.fill(COLOR_SKY_V2)

    # 云朵复用
    draw_pixel_cloud(surface, 8, 20)
    draw_pixel_cloud(surface, 160, 10)
    draw_pixel_cloud(surface, 200, 30)

    # 远景山（深色，矮）
    def draw_mountain(surf, cx, base_y, w, h, color_dark, color_mid):
        for dy in range(h):
            ratio = dy / h
            half_w = int(w * 0.5 * (1 - dy / h))
            pygame.draw.line(surf, color_dark, (cx - half_w, base_y - dy), (cx + half_w, base_y - dy))
        # 高光
        for dy in range(h // 3):
            half_w = int(w * 0.15 * (1 - dy / (h // 3)))
            pygame.draw.line(surf, color_mid, (cx - half_w, base_y - h + dy), (cx + half_w, base_y - h + dy))

    draw_mountain(surface, 60, 155, 80, 55, COLOR_MOUNTAIN_DARK, COLOR_MOUNTAIN_MID)
    draw_mountain(surface, 150, 155, 100, 70, COLOR_MOUNTAIN_DARK, COLOR_MOUNTAIN_MID)
    draw_mountain(surface, 220, 155, 60, 40, COLOR_MOUNTAIN_DARK, COLOR_MOUNTAIN_MID)

    # 草地顶层
    pygame.draw.rect(surface, COLOR_GRASS_TOP, (0, 152, GAME_WIDTH, 6))
    pygame.draw.rect(surface, COLOR_GRASS_DARK, (0, 155, GAME_WIDTH, 3))

    # 土地层
    for by in range(158, GAME_HEIGHT, 10):
        pygame.draw.rect(surface, COLOR_DIRT, (0, by, GAME_WIDTH, 10))
        pygame.draw.line(surface, COLOR_DIRT_DARK, (0, by), (GAME_WIDTH, by), 1)
    ...
    # 管道身体
    pygame.draw.rect(surface, COLOR_PIPE_GREEN, (pipe_x, pipe_y + pipe_top_h, pipe_w, pipe_h))
    pygame.draw.line(surface, COLOR_PIPE_LIGHT, (pipe_x + 3, pipe_y + pipe_top_h), (pipe_x + 3, pipe_y + pipe_top_h + pipe_h), 2)
    pygame.draw.line(surface, COLOR_PIPE_DARK, (pipe_x + pipe_w - 2, pipe_y + pipe_top_h), (pipe_x + pipe_w - 2, pipe_y + pipe_top_h + pipe_h), 2)

    # 管道顶帽
    pygame.draw.rect(surface, COLOR_PIPE_GREEN, (pipe_x - 2, pipe_y, pipe_w + 4, pipe_top_h + 2))
    pygame.draw.line(surface, COLOR_PIPE_LIGHT, (pipe_x, pipe_y + 1), (pipe_x + pipe_w + 2, pipe_y + 1), 1)
    pygame.draw.line(surface, COLOR_PIPE_DARK, (pipe_x - 2, pipe_y + pipe_top_h + 1), (pipe_x + pipe_w + 2, pipe_y + pipe_top_h + 1), 1)

    # 食人花（从管道里长出来）
    stem_x = pipe_x + pipe_w // 2
    stem_top = pipe_y - 22

    # 茎
    pygame.draw.line(surface, COLOR_FLOWER_STEM, (stem_x, pipe_y), (stem_x, stem_top + 8), 3)

    # 花头（圆形红色）
    flower_cx = stem_x
    flower_cy = stem_top
    pygame.draw.circle(surface, COLOR_FLOWER_RED, (flower_cx, flower_cy), 9)
    pygame.draw.circle(surface, COLOR_FLOWER_MOUTH, (flower_cx, flower_cy + 2), 5)

    # 白色斑点
    pygame.draw.circle(surface, COLOR_FLOWER_WHITE, (flower_cx - 4, flower_cy - 3), 2)
    pygame.draw.circle(surface, COLOR_FLOWER_WHITE, (flower_cx + 4, flower_cy - 3), 2)

    # 嘴巴（小横线模拟牙齿）
    pygame.draw.line(surface, COLOR_FLOWER_RED, (flower_cx - 3, flower_cy + 2), (flower_cx + 3, flower_cy + 2), 1)
...
def draw_skip_hint(surface):
    hint = small_font.render("[P] 跳到本Block结束  [E] 结尾  [ESC] 退出", True, (160, 160, 160))
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_p:
                print("[DEBUG] P pressed - skip block")
                return "skip_block"
            if event.key == pygame.K_e:
                print("[DEBUG] E pressed - end experiment")
                return "end"
...
def run_freeze_frame(surface, virtual_screen, trial_idx, block_idx, duration_ms=1200):
    ...
    while pygame.time.get_ticks() - start < duration_ms:
        # 维持 virtual_screen 最后一帧不重绘，直接scale上去
...
def run_game_start_menu(surface, game_width, game_height, scale):
    ...
    if os.path.exists(_p(_FONT_DIR, "SuperMario256.ttf")):
        ...
        HD_FONT_TITLE = safe_sysfont("Arial", 50)
        HD_FONT_TEXT = safe_sysfont("Arial", 35)

    if FONT_PATH:
        PIXEL_FONT_SMALL = pygame.font.Font(FONT_PATH, 12)
    else:
        PIXEL_FONT_SMALL = safe_sysfont("SimHei", 12)

    if os.path.exists(_p(_IMG_DIR, "background.png")):
    ...
    while in_menu:
        ...
        debug_text = PIXEL_FONT_SMALL.render("DEBUG: P=跳到本Block结束  E=结尾  ESC=退出", True, C3)
        surface.blit(debug_text, (game_width * scale // 2 - debug_text.get_width() // 2, 475))
        pygame.display.flip()
        clock.tick(60)
...
for block_idx in range(NUM_BLOCKS):
    ...
    for trial_idx, trial_type in enumerate(trials):
        ...
        # block2 用 v2 背景
        scenery_fn = draw_retro_scenery_v2 if block_idx == 1 else draw_retro_scenery
